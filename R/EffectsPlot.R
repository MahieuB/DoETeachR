...
EffectsPlot=function(formula,design,effects="direct"){
  ...
  if (effects=="direct"){
    ...
    for (f in factor.name){
      grille.f=grille ; grille.f[,f]=seq(-1,1,length.out=500) ; x.f=model.matrix(pseudo.formula,as.data.frame(grille.f))
    ...
    for (i in interactions){
      dans.i=strsplit(i,"[:]")[[1]]
      grille.i=grille ; grille.i[,dans.i[1]]=rep(seq(-1,1,length.out=500),2) ; grille.i[,dans.i[2]]=rep(c(-1,1),each=500)
    ...
    colnames(df.plot)=c("Interaction","Value1","Value2","Response") ; df.plot=as.data.frame(df.plot) ; df.plot[,1]=factor(df.plot[,1],levels = unique(df.plot[,1])) ; df.plot[,2]=as.numeric(df.plot[,2]) ; df.plot[,3]=as.factor(df.plot[,3]) ; df.plot[,4]=as.numeric(df.plot[,4])
